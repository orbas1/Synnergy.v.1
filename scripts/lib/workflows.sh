...
add_run_step() {
  ...
  shift 2
  local -a args=("$@")
  add_step run "$desc" "$workdir" "$(pack_args "${args[@]}")"
}

add_require_commands_step() {
  local desc="$1"
  shift
  add_step require_commands "$desc" "" "$(pack_args "$@")"
}
...
add_python_step() {
  ...
  local workdir="$2"
  local script="$3"
  add_step python "$desc" "$workdir" "$script"
}
...
build_devops_steps() {
  ...
  case "$WF_ACTION" in
    configure_environment)
      ...
      local env_file="$STATE_DIR/devops/environment_${profile}.env"
      local env_content
      env_content=$(cat <<EOF
# Generated by Synnergy workflow launcher
export SYN_PROFILE="$profile"
export SYN_WORKFLOW_STATE_DIR="$STATE_DIR"
export SYN_LOG_DIR="${LOG_FILE:-$DEFAULT_LOG_DIR}"
export SYN_TIMEOUT="$DEFAULT_TIMEOUT"
EOF
)
      add_step write_file "Render environment profile" "$env_file" "$env_content"
      add_step note "Environment profile ready" "$env_file"
      add_step write_file "Environment metadata" "$summary" "{\"profile\":\"$profile\",\"env_file\":\"$env_file\"}"
      ;;
    format_code)
      local gofmt_flag="$(params_get gofmt true)"
      local gofmt_lower="${gofmt_flag,,}"
      add_require_commands_step "Ensure Go toolchain for formatting" go
      if [[ "$gofmt_lower" == "true" || "$gofmt_flag" == "1" ]]; then
        add_run_step "Format Go sources" "$PROJECT_ROOT" go fmt ./...
      else
        add_step note "Go formatting disabled" "$gofmt_flag"
      fi
      add_step write_file "Format plan" "$summary" "{\"gofmt\":\"$gofmt_flag\"}"
      ;;
    install_dependencies)
      local section="$(params_get section all)"
      local section_lower="${section,,}"
      add_require_commands_step "Ensure Go toolchain for dependencies" go
      if [[ "$section_lower" == "all" || "$section_lower" == "go" ]]; then
        add_run_step "Download Go modules" "$PROJECT_ROOT" go mod download
      fi
      if [[ "$section_lower" == "all" || "$section_lower" == "web" ]]; then
        if [[ -f "$PROJECT_ROOT/web/package.json" ]]; then
          add_require_commands_step "Ensure npm for web dependencies" npm
          add_run_step "Install web dependencies" "$PROJECT_ROOT/web" npm install
        else
          add_step note "Web workspace not present" "$PROJECT_ROOT/web"
        fi
      fi
      if [[ "$section_lower" == "all" || "$section_lower" == "docs" ]]; then
        if [[ -f "$PROJECT_ROOT/docs/requirements.txt" ]]; then
          add_require_commands_step "Ensure pip for docs dependencies" pip3
          add_run_step "Install documentation dependencies" "$PROJECT_ROOT/docs" pip3 install -r requirements.txt
        else
          add_step note "Docs requirements file not found" "$PROJECT_ROOT/docs/requirements.txt"
        fi
      fi
    ...
    update_dependencies)
      ...
      add_require_commands_step "Ensure Go toolchain for updates" go
      if [[ -n "$module" && "$module" != "all" ]]; then
        add_run_step "Update Go module $module" "$PROJECT_ROOT" go get -u "$module"
      else
        add_run_step "Update Go modules" "$PROJECT_ROOT" go get -u ./...
      fi
      add_run_step "Tidy Go modules" "$PROJECT_ROOT" go mod tidy
      add_step write_file "Dependency update" "$summary" "{\"module\":\"$module\"}"
      ;;
    cli_tooling)
      local tools="$(params_get tools synctl,vmctl)"
      local bin_dir="$PROJECT_ROOT/bin"
      add_step ensure_directory "Ensure CLI bin directory" "$bin_dir"
      local IFS=','
      ...
      for tool in "${tool_list[@]}"; do
        local trimmed="${tool// /}"
        [[ -z "$trimmed" ]] && continue
        local source_dir="$PROJECT_ROOT/cmd/$trimmed"
        local output_bin="$bin_dir/$trimmed"
        if [[ -d "$source_dir" ]]; then
          add_run_step "Build $trimmed CLI" "$PROJECT_ROOT" go build -trimpath -o "$output_bin" "./cmd/$trimmed"
          add_step note "$trimmed CLI ready" "$output_bin"
        else
          add_step note "$trimmed CLI source not found" "$source_dir"
        fi
      done
    ...
    artifact_checksum)
      ...
      local target_path="$(resolve_path "$path")"
      local script
      script=$(cat <<PY
import hashlib
import json
...
root = pathlib.Path(r"$target_path")
output = pathlib.Path(r"$summary")
checksums = {}
if not root.exists():
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps({"root": str(root), "files": checksums}, indent=2) + "\n", encoding="utf-8")
...
for item in files:
    ...
    with item.open('rb') as fh:
        for chunk in iter(lambda: fh.read(65536), b''):
            digest.update(chunk)
    checksums[rel] = digest.hexdigest()

output.parent.mkdir(parents=True, exist_ok=True)
output.write_text(json.dumps({"root": str(root), "files": checksums}, indent=2) + "\n", encoding="utf-8")
PY
)
      add_python_step "Compute artifact checksums" "$PROJECT_ROOT" "$script"
      add_step write_file "Checksum manifest" "$summary" "{\"path\":\"$path\"}"
      ;;
    script_completion_setup)
      local shell="$(params_get shell bash)"
      local shell_lower="${shell,,}"
      local completion_dir="$STATE_DIR/devops/completions"
      local completion_file="$completion_dir/${shell_lower}.sh"
      local base_commands="consensus cross-chain dao network wallet node treasury monitoring deployment devops testing"
      local snippet
      case "$shell_lower" in
        bash|zsh)
          snippet=$(cat <<EOS
# Synnergy CLI completion for $shell_lower
_synnergy_complete() {
  local cur prev
  COMPREPLY=()
...
complete -F _synnergy_complete synnergy
EOS
)
          ;;
        fish)
          snippet=$(cat <<EOS
# Synnergy CLI completion for fish shell
complete -c synnergy -n 'not __fish_seen_subcommand_from $base_commands' -a '$base_commands'
EOS
)
          ;;
        *)
          snippet=$(cat <<EOS
# Synnergy CLI completion placeholder for $shell
# Populate with shell-specific completion commands as required.
EOS
)
          ;;
      esac
      add_step ensure_directory "Ensure completion directory" "$completion_dir"
      add_step write_file "Install completion script" "$completion_file" "$snippet"
      add_step note "Completion script generated" "$completion_file"
      add_step write_file "Completion setup" "$summary" "{\"shell\":\"$shell\",\"completion\":\"$completion_file\"}"
      ;;
    script_launcher)
...
build_testing_steps() {
  ...
  case "$WF_ACTION" in
    contract_static_analysis)
      ...
      add_require_commands_step "Ensure Go toolchain for static analysis" go
      add_run_step "Run go vet for $suite" "$PROJECT_ROOT" go vet ./...
      add_step write_file "Static analysis" "$summary" "{\"suite\":\"$suite\"}"
      ;;
    contract_test_suite)
      local package="$(params_get package ./...)"
      add_require_commands_step "Ensure Go toolchain for contract tests" go
      add_run_step "Run contract tests" "$PROJECT_ROOT" go test "$package"
      add_step write_file "Contract tests" "$summary" "{\"package\":\"$package\"}"
      ;;
    contract_coverage_report)
      local output="$(params_get output reports/coverage.out)"
      local coverage_path="$(resolve_path "$output")"
      local coverage_dir="$(dirname "$coverage_path")"
      add_step ensure_directory "Ensure coverage directory" "$coverage_dir"
      add_require_commands_step "Ensure Go toolchain for coverage" go
      add_run_step "Generate coverage report" "$PROJECT_ROOT" go test -coverprofile="$coverage_path" ./...
...
allowed = {"solidity", "vyper", "rust", "move"}
unsupported = [lang for lang in languages if lang not in allowed]
payload = {"languages": languages, "unsupported": unsupported}
target = pathlib.Path(r"$summary")
...
PY
      ...
      add_python_step "Validate contract language compatibility" "$PROJECT_ROOT" "$script"
      ;;
    integration_test_suite)
      local focus="$(params_get focus all)"
      add_require_commands_step "Ensure Go toolchain for integration tests" go
      if [[ "$focus" == "all" ]]; then
        add_run_step "Run integration tests" "$PROJECT_ROOT" go test ./...
      else
        add_run_step "Run focused integration tests" "$PROJECT_ROOT" go test ./... -run "$focus"
      fi
    ...
    fuzz_testing)
      local package="$(params_get package ./...)"
      local duration="$(params_get duration 30)"
      add_require_commands_step "Ensure Go toolchain for fuzz testing" go
      add_run_step "Run go fuzz" "$PROJECT_ROOT" go test "$package" -run=Fuzz -fuzz=. -fuzztime="${duration}s"
      add_step write_file "Fuzz testing" "$summary" "{\"package\":\"$package\",\"duration\":$duration}"
      ;;
    performance_regression)
      local benchmark="$(params_get benchmark default)"
      add_require_commands_step "Ensure Go toolchain for benchmarks" go
      add_run_step "Run performance benchmarks" "$PROJECT_ROOT" go test -run=NONE -bench "$benchmark" -benchmem ./...
      add_step write_file "Performance regression" "$summary" "{\"benchmark\":\"$benchmark\"}"
      ;;
    stress_test_network)
      local duration="$(params_get duration 120)"
      add_run_step "Execute network stress harness" "$PROJECT_ROOT/scripts" ./network_harness.sh --plan --set duration="$duration"
      add_step write_file "Stress test" "$summary" "{\"duration\":$duration}"
      ;;
    benchmarks)
      local suite="$(params_get suite core)"
      add_require_commands_step "Ensure Go toolchain for benchmark suite" go
      add_run_step "Run benchmark suite" "$PROJECT_ROOT" go test -run=NONE -bench . -benchmem ./...
      add_step write_file "Benchmarks" "$summary" "{\"suite\":\"$suite\"}"
      ;;
    gui_wallet_test)
      local profile="$(params_get profile smoke)"
      if [[ -f "$PROJECT_ROOT/web/package.json" ]]; then
        add_require_commands_step "Ensure npm for wallet tests" npm
        add_run_step "Run wallet UI tests" "$PROJECT_ROOT/web" npm test -- --watch=false
      else
        add_step note "Web workspace not present for GUI tests" "$PROJECT_ROOT/web"
      fi
      add_step write_file "GUI wallet test" "$summary" "{\"profile\":\"$profile\"}"
      ;;
    scripts_test)
      local run_pattern="$(params_get run .)"
      add_require_commands_step "Ensure Go toolchain for script tests" go
      if [[ "$run_pattern" == "." ]]; then
        add_run_step "Run script package tests" "$PROJECT_ROOT" go test ./scripts
      else
        add_run_step "Run targeted script package tests" "$PROJECT_ROOT" go test ./scripts -run "$run_pattern"
      fi
      add_step write_file "Scripts go test" "$summary" "{\"run\":\"$run_pattern\"}"
      ;;
    release_sign_verify)
      local artifact="$(params_get artifact dist/release.tar.gz)"
      local artifact_path="$(resolve_path "$artifact")"
      local script
      script=$(cat <<PY
import hashlib
import json
...
artifact = pathlib.Path(r"$artifact_path")
output = pathlib.Path(r"$summary")
payload = {"artifact": str(artifact), "exists": artifact.exists(), "sha256": None}
...
PY
      ...
      add_python_step "Prepare release artifact verification" "$PROJECT_ROOT" "$script"
      ;;
    *)
...
execute_step() {
  ...
  case "$type" in
    ...
    run)
      unpack_args "$data2"
      local -a cmd=("${WF_UNPACKED_ARGS[@]}")
      local run_enabled="${SYN_WORKFLOW_EXECUTE_CMDS:-0}"
      local workdir="$data1"
      local printable
      if ((${#cmd[@]} == 0)); then
        status="failed"
        ...
        printable="${cmd[*]}"
        if [[ -n "$workdir" ]]; then
          printable="(cd $workdir && ${cmd[*]})"
        fi
        if is_plan_mode || [[ "$run_enabled" != 1 ]]; then
          status="planned"
          message="$printable"
        else
          local previous_dir=""
    ...
    require_commands)
      unpack_args "$data2"
      local -a required=("${WF_UNPACKED_ARGS[@]}")
      if is_plan_mode; then
        status="planned"
      ...
      ;;
    python)
      local run_python="${SYN_WORKFLOW_EXECUTE_CMDS:-0}"
      local workdir="$data1"
      local script="$data2"
      if is_plan_mode || [[ "$run_python" != 1 ]]; then
        status="planned"
        message="python3 (inline script)"
      else
        if ! command -v python3 >/dev/null 2>&1; then
          ...
          script_file="$(mktemp)"
          printf '%s\n' "$script" >"$script_file"
          if with_timeout "$desc" python3 "$script_file"; then
            status="ok"
            message="python3 $script_file"
          else
            status="failed"
            message="python3 $script_file"
          fi
