...
## Treasury-Controlled Burns and Rollbacks
Stage 80 supplements dispute tooling with a governed burn path so the Synthron treasury can retire funds that were minted erroneously or clawed back through authority review. The ledger now exposes a dedicated `Burn` method that validates ownership, enforces balance sufficiency and updates the UTXO view without crediting another account, guaranteeing that monetary supply is reduced deterministically when invoked【F:core/ledger.go†L164-L186】. The Synthron treasury wraps this primitive with policy controls: the CLI, virtual machine and web dashboards all funnel burn requests through the `SynthronTreasury` service, which records events, refreshes telemetry and streams hooks to the VM so auditors can see when supply adjustments occur【F:treasury/synthron_treasury.go†L146-L320】【F:cli/coin.go†L36-L130】. Operator permissions now gate every reversal. Only identities granted through `AuthorizeOperator` may trigger issue, burn, transfer or consensus-bridge flows, and the guardian wallet cannot be revoked, preventing accidental loss of monetary control while still allowing rotation of day-to-day operators【F:treasury/synthron_treasury.go†L98-L215】【F:treasury/synthron_treasury.go†L438-L520】. The function web mirrors these safety rails by surfacing operator lists, gas coverage and governed burn forms, ensuring UI-driven reversals respect the same permissions as CLI and contract calls【F:web/pages/index.js†L1-L210】. This orchestration ensures that emergency reversals, regulatory takedowns and scheduled token retirements remain auditable, permissioned and aligned with consensus.
